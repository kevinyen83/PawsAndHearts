...
export default function PetCarousel() {
    const [petHighlight, setPetHightlight] = useState<PetHighlight[]>(Pets_Highlight);
    const [startIndex, setStartIndex] = useState(0);

    const handleClickLeft = () => {
        const newIndex = (startIndex - 3 + petHighlight.length) % petHighlight.length;
                        ...
                        <div key={pet.id} className='relative flex flex-col m-2 text-gray-700 bg-white shadow-md rounded-xl w-full sm:w-72 md:w-72 lg:w-72 xl:w-72'>
                            <div className='w-full p-6'>
                                <div className='relative h-56 overflow-hidden rounded-t-xl'>
                                    <Image className='object-cover w-full h-full rounded-t-xl' src={pet.image} alt={pet.name} width={600} height={400} data-testid='cart-item-image' />
                                </div>
                                <div className='p-6'>
