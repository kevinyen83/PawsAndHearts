...
export default function Favorites({ showFavorites, setShowFavorites, favoritesItems, selectedPet, setIsFavoritesEmpty, removeItem, toggleCardDetailPopup }: FavoritesProps) {
    ...
    <div key={selectedPet.id} className='flex items-center justify-between border-b border-gray-200 py-4'>
      <div className='flex items-center'>
        <div className='rounded-full overflow-hidden w-12 h-12 mr-4'>
          <img className='w-full h-full object-cover' src={selectedPet.image} alt={selectedPet.name} data-testid='favorites-side-item-img'/>
        </div>
        <div className='flex flex-col item-left'>
          <p className='text-left' data-testid='favorites-side-item-name'>Name: {selectedPet.name}</p>
          <p className='text-left' data-testid='favorites-side-item-age'>Age: {selectedPet.age}</p>
  ...
  return (
    <Transition.Root show={showFavorites} as={Fragment}>
      <Dialog as='div' className='fixed inset-0 overflow-hidden z-50' onClose={setShowFavorites}>
        <Transition.Child
          as={Fragment}
              ...
              <div className='w-screen max-w-md'>
                <div className='flex flex-col h-full bg-white shadow-xl'>
                  <div className='flex items-center justify-between px-4 py-6 border-b border-gray-200'>
                    <Dialog.Title className='text-lg font-medium text-gray-900' data-testid='favorites-side-header'>Favorites</Dialog.Title>
                    <button type='button' className='text-gray-400 hover:text-gray-500' onClick={() => setShowFavorites(false)}>
